/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Loaded only on /admin (not on the public landing page).
───────────────────────────────────────────────────────────────────────────── */

// ── Context data injected by the server via data-attributes ──────────
...
// ════════════════════════════════════════════════════════════════════════════════
// TOAST — auto-hide and programmatic creation
// ════════════════════════════════════════════════════════════════════════════════

(function initToast() {
  const toast = document.getElementById('admin-toast');
  if (!toast) return;
  // Appears already visible; hide after 4 s
...
/**
 * Shows an ephemeral toast in the content area (called from JS).
...
/**
 * Called by the "Delete" button in the businesses table.
 * Creates a dynamic form and submits it after confirmation.
 */
function deleteBusiness(btn) {
  const { businessId, businessName } = btn.dataset;
  if (!confirm(
    `Delete the business "${businessName}" and ALL its tags?\n\n` +
...
(function initBulkActions() {
  ...
  cbAll.addEventListener('change', () => {
    // Only affects visible (non-filtered) rows
  ...
  async function bulkAction(endpoint, confirmMsg, successKey, successLabel) {
    ...
    if (ids.length === 0) { showToast('error', 'No tags selected.'); return; }
  ...
  document.getElementById('btn-bulk-reset')?.addEventListener('click', () => {
    ...
    bulkAction(
      ...
      `Reset ${n} tag(s) to "available"?\n\nThe claim date will be cleared.`,
      'reset',
      'tag(s) reset'
    );
  });
...
(function initFilter() {
  ...
  input.addEventListener('input', () => {
    ...
    // Uncheck all when filtering to avoid hidden selections
    if (cbAll) {
      cbAll.checked       = false;
...
/**
 * Called by the "Copy" button on each tag row.
 */
