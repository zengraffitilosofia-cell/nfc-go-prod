...
(function migrateValidatorCredentials() {
  ...
  console.log('\n[NFC GO] Generating validator credentials for existing businesses:');
  ...
  db.transaction(() => {
    missing.forEach(b => {
      ...
      console.log(`  → "${b.name}"  username: ${b.slug}  password: ${pass}`);
    });
  })();
  console.log('[NFC GO] Save these passwords: they cannot be recovered later.\n');
})();
...
app.get('/c/:tag_code', (req, res) => {
  ...
  const tag = db
    .prepare(
      `SELECT t.*, b.name AS business_name, b.logo_url, COALESCE(t.reward_text,b.reward_text) AS reward_text, b.slug
       FROM nfc_tags t
...
app.post('/claim/:tag_code', claimLimiter, (req, res) => {
  ...
  const claim = db.transaction(() => {
    ...
    if (tag.status !== 'active')
      return { ok: false, error: 'This prize has already been claimed.', status: 409 };
...
app.post('/v/:slug/validate', requireValidator, (req, res) => {
  ...
  if (!tag) {
    return res.json({ success: false, message: 'Code not found for this business.' });
  }

  if (tag.status === 'redeemed') {
    return res.json({ success: false, message: 'This code has already been redeemed.' });
  }
  if (tag.status === 'active') {
    return res.json({ success: false, message: 'This code has not been claimed by the customer yet.' });
  }
  if (tag.status === 'disabled') {
    return res.json({ success: false, message: 'This code is disabled.' });
  }
  ...
  res.json({ success: true, message: `Code ${tag.discount_code} redeemed successfully.` });
});
...
app.post('/admin/businesses', requireAdmin, upload.single('logo_file'), (req, res) => {
  const { name, slug, validator_username, validator_password } = req.body;
  if (!name || !slug) { return redirectFlash(res,'/admin','error','Name and slug are required.'); }
  ...
  try {
    db.prepare(
      `INSERT INTO businesses (name, slug, logo_url, validator_username, validator_password) VALUES (?, ?, ?, ?, ?)`
    ).run(name, cleanSlug, logo_url, vUsername, hashedPass);
    ...
    if (autoGenerated) {
      return res.redirect(
        `/admin?ok=${encodeURIComponent(`Business "${name}" created successfully`)}&creds_u=${encodeURIComponent(vUsername)}&creds_p=${encodeURIComponent(plainPass)}`
      );
    }
    redirectFlash(res, '/admin', 'success', `Business "${name}" created successfully`);
  } catch (e) {
    if (e.message.includes('UNIQUE')) {
      return redirectFlash(res, '/admin', 'error', `The slug "${cleanSlug}" or username "${vUsername}" already exist.`);
    }
    throw e;
  }
});

app.post('/admin/businesses/:id/edit', requireAdmin, upload.single('logo_file'), (req, res) => {
  const current = db.prepare(`SELECT * FROM businesses WHERE id=?`).get(req.params.id);
  const { name, active, validator_username, validator_password } = req.body;
  const logo_url=req.file?('/uploads/'+req.file.filename):current.logo_url;
  ...
  try {
    ...
    if ((validator_password || '').trim()) {
      ...
      info = db.prepare(
        `UPDATE businesses SET name=?, logo_url=?, active=?, validator_username=?, validator_password=? WHERE id=?`
      ).run(name, logo_url, active === '1' ? 1 : 0, vUsername, hashed, req.params.id);
    } else {
      info = db.prepare(
        `UPDATE businesses SET name=?, logo_url=?, active=?, validator_username=? WHERE id=?`
      ).run(name, logo_url, active === '1' ? 1 : 0, vUsername, req.params.id);
    }

    if (info.changes === 0) return res.redirect('/admin');
    redirectFlash(res, `/admin/businesses/${req.params.id}`, 'success', 'Changes saved');
  } catch (e) {
    if (e.message.includes('UNIQUE')) {
      return redirectFlash(res, `/admin/businesses/${req.params.id}`, 'error', `The username "${vUsername}" is already in use by another business.`);
    }
    throw e;
  }
});
...
app.post('/admin/businesses/:id/delete', requireAdmin, (req, res) => {
  ...
  if (!business) return redirectFlash(res, '/admin', 'error', 'Business not found.');
  ...
  redirectFlash(res, '/admin', 'success', `Business "${business.name}" deleted`);
});
...
app.post('/admin/businesses/:id/generate', requireAdmin, (req, res) => {
  ...
  const reward_text=req.body.reward_text||'';

  const insert = db.prepare(
    `INSERT INTO nfc_tags (tag_code, business_id, discount_code, reward_text) VALUES (?, ?, ?, ?)`
  );
  db.transaction(() => {
    for (let i = 0; i < count; i++) {
      const num = String(startAt + i).padStart(3, '0');
      insert.run(uuidv4(), business.id, `${prefix}-${num}`, reward_text);
    }
  })();

  redirectFlash(res, `/admin/businesses/${req.params.id}`, 'success', `${count} tags generated`);
});
...
app.post('/admin/businesses/:id/reset-all', requireAdmin, (req, res) => {
  ...
  redirectFlash(
    ...
    'success',
    `${result.changes} tag(s) reset to "available"`
  );
});
...
app.get('/admin/businesses/:id/export.csv', requireAdmin, (req, res) => {
  ...
  const header = [
    csvCell('Discount code'),
    csvCell('Tag UUID'),
    csvCell('Status'),
    csvCell('Claim date'),
    csvCell('Redemption date'),
    csvCell('NFC URL'),
  ].join(',');
  ...
  const rows = tags.map(t => [
    ...
    csvCell(t.tag_code),
    csvCell(
      t.status === 'active'    ? 'Available' :
      t.status === 'claimed'   ? 'Claimed'  :
      t.status === 'redeemed'  ? 'Redeemed'   : 'Disabled'
    ),
  ...
  const filename = `${business.slug}-tags.csv`;
...
app.post('/admin/businesses/:id/tags/bulk-delete', requireAdmin, (req, res) => {
  ...
  if (ids.length === 0) return res.status(400).json({ error: 'Invalid IDs.' });
...
app.listen(PORT, () => {
  console.log(`NFC GO running on ${BASE_URL}`);
});
